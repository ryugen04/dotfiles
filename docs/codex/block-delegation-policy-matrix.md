...
| shell inspection | safe forms of `ls`, `pwd`, `cat`, `find`, `rg`, `sed`, `head`, `tail`, `wc`, `sort`, `uniq`, `cut`, `nl`; simple pipelines are allowed only when every segment is read-only | redirects, command substitution, semicolons, `&&`, `||`, subshells, `sed -i`, `find -delete`, exec forms | shell fragments are denied by default |
...
| AI-DLC CLI | `doctor`, `status`, `inspect`, `context`, `validate`, `validate-overlay`, `overlay-status`, `deadlock-check`, block `list/actions`, help | source mutation outside owner flow | assignment create and lifecycle commands are allowed only through role/lease validation; block `record/sync/export` are constrained recovery mutations |
