...
| キー | 動作 |
|------|------|
| `Ctrl+a > p` | kagent Agent Lens を kitty quick-access-terminal で開く |

`kittens/kagent_quick_access.py` は `kagent quick-access` を実行する。
通常運用では、先に kagent repo で binary をインストールしておく。

```sh
cd "$HOME/dev"/projects/kagent
cargo install --path crates/kagent-cli --locked --force
command -v kagent
```

開発版を直接使う場合だけ、環境変数でコマンドを差し替える。
