...
非自明な計画レビューは `.careflow` の PLAN/ORDER/RESULT を正本にします。
...
Claude が Plan を作る場合も、実装正本は `.careflow/cases/<case_id>/PLAN.md`。Codex へ渡すときは agmsg で固定 Handoff Header と path を送る。

Codex から Plan 欠陥が戻った場合は、Claude が PLAN を修正し、controller が plan hash と ORDER を更新してから再委託する。chat内の説明だけで再実行しない。

通常編集経路が壊れている場合は、Codex に OS 権限修復を求めず patch artifact を返させる。
...
レビュー結果を保存:
- 正本: `.careflow/cases/<case_id>/results/<order_id>.result.md` または `reviews/`
- 証跡: `.careflow/cases/<case_id>/evidence/`
- 一時メモのみ: `.claude/work/plans/{plan-name}/plan-review.md`
...
- 計画の変更は提案のみ、直接編集はしない
- careflow context がある場合は `EXPECTED_RESULT_PATH` にレビュー結果を残す
