...
For Markdown files: match the existing document's formatting when inserting the verification summary.

**Target file** — determine what to verify from `$1`:
- Explicit path: verify that specific file (`.md` or any text document)
- No argument: verify the most recently modified `.md` file in `~/.agent/diagrams/` (`ls -t ~/.agent/diagrams/*.md | head -1`)

Auto-detect the document type and adjust the verification strategy:
- **Review pages** (diff-review, plan-review, project-recap): detect from page content, verify against the git ref or plan file the review was based on
...
- Preserve layout, formatting, Mermaid diagrams (unless they contain factual errors in node labels or edge descriptions)
- Preserve heading structure and document organization

**Phase 4: Add verification summary.**

Append a verification section at the end of the document:
...
### 修正内容
- `processCleanup` → `runCleanup` に修正 (`worker.ts:45` の実際の関数名に合わせて)
- ファイル数 12 → 14 に修正 (`git diff --stat` の出力と照合)
- ...
```

**Phase 5: Report.** Tell the user what was checked, what was corrected, and the file path. If nothing needed correction, say so — the verification still has value as confirmation.
