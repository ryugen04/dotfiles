...
8. cmux の `browser open-split` で右ペインに `http://localhost:4967/` を開く
...
OK surface=surface:N pane=pane:N placement=split  (または placement=reuse)
```

`difit server started` と `OK surface=... placement=split/reuse` の 2 行が出ていれば成功。
