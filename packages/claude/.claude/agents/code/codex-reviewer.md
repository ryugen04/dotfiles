...
1. 対象差分または対象ファイル群を特定
2. `.careflow` の PLAN/ORDER/EXPECTED_RESULT を確認
3. Codex CLI 実行（`review` プロファイル = gpt-5.5 / read-only）
4. 結果を EXPECTED_RESULT_PATH または `.careflow/.../reviews/` に整形
...
- careflow context がある場合、`.claude/work` ではなく `.careflow` result/review/evidence を正本にする
